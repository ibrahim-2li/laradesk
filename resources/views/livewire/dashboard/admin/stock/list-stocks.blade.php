<div>
    <div class="bg-white rounded-[4px] shadow-sm border border-gray-100 mt-20">
        <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center">
            <h2 class="text-xl font-semibold text-gray-700">{{ __('Stock') }}</h2>
                    ...
                    @forelse($stocks as $stock)
                        <tr class="hover:bg-gray-50/50 transition-colors group">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-semibold text-gray-600">{{ $stock->name }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="bg-gray-100 text-gray-800 text-xs font-medium px-2.5 py-0.5 rounded border border-gray-200">
                                    {{ $stock->brands->name ?? __('Unknown') }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if ($stock->quantity > 0)
                                    <span
                                        class="text-sm border border-green-200 bg-green-50 text-green-600 font-semibold px-3 py-1 rounded-md">
                                        {{ $stock->quantity }}
                                    </span>
                                @else
                                    <span
                                        class="text-sm border border-red-200 bg-red-50 text-red-600 font-semibold px-3 py-1 rounded-md">
                                        {{ __('Out of Stock') }}
                                    </span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="relative" x-data="{ open: false }">
                                    ...
                                    <div x-show="open"
                                        ...
                                        style="display: none;">
                                        <div class="py-1">
                                            <button wire:click="edit({{ $stock->id }})" @click="open = false"
