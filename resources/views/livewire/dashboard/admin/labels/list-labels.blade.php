<div>
    <div class="mb-6 flex justify-between items-center">
        <h1 class="text-2xl font-bold text-gray-800">{{ __('Labels') }}</h1>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="p-4 border-b border-gray-200">
            <div class="w-full md:w-1/3">
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="{{ __('Search labels...') }}"
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="h-6 w-6 rounded border border-gray-200 mr-2"
                                        style="background-color: {{ $label->color }}"></div>
                                    <span class="text-sm text-gray-500">{{ $label->color }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $label->tickets_count + $label->orders_count }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                ...
                                <button wire:click="delete({{ $label->id }})"
                                    wire:confirm="{{ __('Are you sure you want to delete this label?') }}"
            ...
            <div class="bg-white rounded-lg shadow-xl w-full max-w-lg mx-4">
                <div class="px-6 py-4 border-b">
                    <h3 class="text-lg font-semibold text-gray-900">
                        {{ $editingId ? __('Edit Label') : __('Create New Label') }}
                    </h3>
                </div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center mt-1">
                                <input wire:model="color" type="color"
                                    class="h-10 w-20 border-gray-300 shadow-sm rounded-l-md overflow-hidden cursor-pointer"
                                    required>
                                <input wire:model="color" type="text"
                                    class="form-input block w-full rounded-r-md border-l-0 border-gray-300 shadow-sm uppercase"
