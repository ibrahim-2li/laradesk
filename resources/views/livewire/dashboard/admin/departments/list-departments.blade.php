    <div class="bg-white rounded-[4px] shadow-sm border border-gray-100 mt-20">
        <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center">
            <h2 class="text-xl font-semibold text-gray-700">{{ __('Departments') }}</h2>
                    ...
                    @forelse($departments as $department)
                        <tr class="hover:bg-gray-50/50 transition-colors group">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-semibold text-gray-600">{{ $department->name }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="text-sm text-gray-500 font-medium">
                                    {{ $department->public ? __('Public') : __('Internal') }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex -space-x-2 overflow-hidden">
                                    ...
                                    @else
                                        @foreach ($department->agent as $agent)
                                            <img class="inline-block h-8 w-8 rounded-full ring-2 ring-white object-cover"
                                                src="{{ $agent->getAvatar() }}" alt="{{ $agent->name }}"
                                                title="{{ $agent->name }}">
                                        @endforeach
                                        @if ($department->agent_count > 5)
                                            <span
                                                ...
                                                +{{ $department->agent_count - 5 }}
                                            </span>
                                        @endif
                                        @if ($department->agent_count === 0)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="relative" x-data="{ open: false }">
                                    ...
                                    <div x-show="open"
                                        ...
                                        style="display: none;">
                                        <div class="py-1">
                                            <button wire:click="edit({{ $department->id }})" @click="open = false"
