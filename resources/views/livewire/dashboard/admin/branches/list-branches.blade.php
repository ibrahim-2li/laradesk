<div>
    <div class="bg-white rounded-[4px] shadow-sm border border-gray-100 mt-20">
        <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center">
            <h2 class="text-xl font-semibold text-gray-700">{{ __('Branches') }}</h2>
                    ...
                    @forelse($branches as $branch)
                        <tr class="hover:bg-gray-50/50 transition-colors group">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-semibold text-gray-600">{{ $branch->name }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="text-sm text-gray-500 font-medium">
                                    {{ $branch->public ? __('Public') : __('Internal') }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex -space-x-2 overflow-hidden">
                                    @if ($branch->all_agents)
                                        <span
                                            class="inline-flex items-center justify-center h-8 w-8 rounded-full ring-2 ring-white bg-blue-100 text-blue-600 font-bold text-xs"
                                            title="{{ __('All Agents') }}">
                                            ALL
                                        </span>
                                    @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if ($branch->public)
                                    <span
                                        class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-[4px] bg-purple-100 text-purple-700 uppercase tracking-wide">
                                        {{ __('Active') }}
                                    </span>
                                @else
                                    <span
                                        class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-[4px] bg-gray-100 text-gray-600 uppercase tracking-wide">
                                        {{ __('Private') }}
                                    </span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="relative" x-data="{ open: false }">
                                    ...
                                    <div x-show="open"
                                        ...
                                        style="display: none;">
                                        <div class="py-1">
                                            <button wire:click="edit({{ $branch->id }})" @click="open = false"
            ...
            <div class="bg-white rounded-lg shadow-xl w-full max-w-lg mx-4">
                <div class="px-6 py-4 border-b">
                    <h3 class="text-lg font-semibold text-gray-900">
                        {{ $editingId ? __('Edit Branch') : __('Create New Branch') }}
                    </h3>
                </div>
                <form wire:submit.prevent="store">
                    <div class="px-6 py-4 space-y-4">
                        ...
                        <div class="flex items-center">
                            ...
                            <label class="ml-2 block text-sm text-gray-900">{{ __('Public') }}</label>
                        </div>
                        ...
                        <div class="flex items-center">
                            <input wire:model="all_agents" type="checkbox"
                                class="form-checkbox h-4 w-4 text-blue-600 transition duration-150 ease-in-out">
                            <label class="ml-2 block text-sm text-gray-900">{{ __('All Agents Access') }}</label>
                        </div>
                    </div>
