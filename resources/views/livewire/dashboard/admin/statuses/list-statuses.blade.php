<div>
    <div class="bg-white rounded-[4px] shadow-sm border border-gray-100 mt-20">
        <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center">
            <h2 class="text-xl font-semibold text-gray-700">{{ __('Statuses') }}</h2>
                    ...
                    @forelse($statuses as $status)
                        <tr class="hover:bg-gray-50/50 transition-colors group">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-semibold text-gray-600">{{ $status->name }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-2">
                                    <div class="h-6 w-6 rounded border border-gray-200"
                                        style="background-color: {{ $status->color }}"></div>
                                    <span class="text-sm text-gray-500 font-mono">{{ $status->color }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                <span class="bg-gray-100 text-gray-600 py-1 px-3 rounded-full text-xs font-medium">
                                    {{ $status->tickets_count }} {{ __('Uses') }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                <div class="relative inline-block text-left" x-data="{ open: false }">
                                    ...
                                    <div x-show="open"
                                        ...
                                        style="display: none;">
                                        <div class="py-1">
                                            <button wire:click="edit({{ $status->id }})" @click="open = false"
            ...
            <div class="bg-white rounded-lg shadow-xl w-full max-w-lg mx-4">
                <div class="px-6 py-4 border-b">
                    <h3 class="text-lg font-semibold text-gray-900">
                        {{ $editingId ? __('Edit Status') : __('Create New Status') }}
                    </h3>
                </div>
