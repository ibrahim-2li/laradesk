    ...
    <div class="mt-20 mb-8 flex flex-col md:flex-row md:justify-between md:items-start gap-4">
        <div>
            <div class="flex items-center gap-3 mb-2">
                <span
                    class="px-2.5 py-1 rounded-[4px] text-xs font-mono text-gray-600 bg-gray-100 border border-gray-200">#{{ $ticket->id }}</span>
                <h1 class="text-2xl font-bold tracking-tight text-gray-900">{{ $ticket->subject }}</h1>
                <span
                    class="px-2.5 py-0.5 rounded-full text-xs font-medium {{ optional($ticket->status)->name == 'Open' ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-gray-100 text-gray-700 border border-gray-200' }}">
            ...
            <div class="flex flex-wrap items-center gap-4 text-sm text-gray-500">
                <span class="flex items-center gap-2">
                    <img class="h-6 w-6 rounded-full border border-gray-200"
                        src="{{ $ticket->user->profile_photo_url ?? 'https://ui-avatars.com/api/?name=' . urlencode($ticket->user->name) }}"
                        alt="">
                ...
                <div class="space-y-6">
                    @foreach ($replies as $reply)
                        <div class="flex {{ $reply->user_id === $ticket->user_id ? 'justify-start' : 'justify-end' }}">
                            <div
                                class="flex flex-col {{ $reply->user_id === $ticket->user_id ? 'items-start' : 'items-end' }} max-w-[85%]">
                                ...
                                <div
                                    class="px-5 py-3.5 rounded-2xl shadow-sm text-sm border {{ $reply->user_id === $ticket->user_id ? 'bg-gray-50 text-gray-800 rounded-tl-none border-gray-200' : 'bg-blue-600 text-white rounded-tr-none border-blue-600' }}">
                                    <div
                                        class="prose prose-sm prose-invert {{ $reply->user_id === $ticket->user_id ? 'text-gray-700' : 'text-blue-50' }}">
                                        {!! nl2br(e($reply->body)) !!}
                                    </div>

                                    @if ($reply->ticketAttachments->count() > 0)
                                        <div
                                            class="mt-3 pt-3 border-t {{ $reply->user_id === $ticket->user_id ? 'border-gray-200' : 'border-blue-500/30' }}">
                                            <div class="flex flex-wrap gap-2">
                                                @foreach ($reply->ticketAttachments as $attachment)
                                                    <a href="{{ $attachment->url() }}" target="_blank"
                                                        class="group relative block overflow-hidden rounded-lg border {{ $reply->user_id === $ticket->user_id ? 'border-gray-200' : 'border-blue-500' }}">
                ...
                <form wire:submit.prevent="reply">
                    ...
                    <div class="mb-4">
                        <label for="ticket-attachments"
                            ...
                            {{ __('Attachments (Images only)') }}
                        </label>
                        <input type="file" id="ticket-attachments" wire:model="attachments" multiple accept="image/*"
                        ...
                        <!-- Preview -->
                        <div wire:loading wire:target="attachments" class="mt-2 text-xs text-blue-500">
                            {{ __('Uploading...') }}
                        </div>
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-white rounded-[4px] shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-4 py-3 border-b border-gray-100 bg-gray-50/50">
                    <h3 class="text-xs font-bold text-gray-900 uppercase tracking-wider">{{ __('Ticket Details') }}
                    </h3>
                </div>
                <div class="p-4 space-y-5">
                    <div>
                        ...
                        <dd class="text-sm font-semibold text-gray-900">
                            {{ $ticket->department->name ?? __('No Department') }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1.5">
                            {{ __('Priority') }}</dt>
                        <dd class="text-sm font-medium text-gray-900 flex items-center gap-2">
                            ...
                            @endif
                            <span class="font-semibold">{{ $ticket->priority->name ?? __('No Priority') }}</span>
                        </dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1.5">
                        ...
                        <dd class="text-sm font-medium text-gray-900 flex items-center gap-2">
                            @if ($ticket->agent)
                                <img class="h-6 w-6 rounded-full border border-gray-100"
                                    src="{{ $ticket->agent->profile_photo_url ?? 'https://ui-avatars.com/api/?name=' . urlencode($ticket->agent->name) }}"
                                    alt="">
                                <span class="font-semibold">{{ $ticket->agent->name }}</span>
                            @else
                    ...
                    <div class="pt-4 border-t border-gray-100">
                        <div class="text-xs text-gray-400 flex items-center gap-1">
                            ...
                            {{ __('Last Updated:') }} <span
                                class="text-gray-500 font-medium">{{ $ticket->updated_at->diffForHumans() }}</span>
                        </div>
                    </div>
                </div>
            </div>

            <button wire:click="delete" wire:confirm="{{ __('Are you sure you want to delete this ticket?') }}"
