<div>
                ...
                <div class="flex {{ $reply->user_id === auth()->id() ? 'justify-end' : 'justify-start' }}">
                    <div
                        ...
                        <div
                            ...
                            @if ($reply->orderAttachments && $reply->orderAttachments->count() > 0)
                                <div
    ...
    <!-- Reply Area -->
    @if ($order->orders_status_id == 3)
        <div class="bg-gray-50 border border-gray-200 rounded-xl p-6 text-center">
            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <h3 class="mt-2 text-sm font-medium text-gray-900">{{ __('Order Completed') }}</h3>
            <p class="mt-1 text-sm text-gray-500">
                {{ __('This order has been completed/delivered. Replies are disabled.') }}</p>
        </div>
    @else
