<template>
    <div id="app">
      <div class="py-10">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 px-2">
          ...
          <div class="max-w-3xl mx-auto px-1 sm:px-6 lg:px-1">
                    ...
                    <div class="col-span-3">
                      ...
                      <form @submit.prevent="saveOrder">
                        ...
                        <tr v-for="(item, index) in order.orderItems" :key="index">
                          <td>
                            <input-select id="stocks" v-model="item.item" :placeholder="$t('Select Items')" :options="filteredItemsList(index)" option-label="name" required />
                          </td>
                          <td>
                            <input
                              class="form-input block w-full transition duration-150 ease-in-out sm:text-sm sm:leading-5"
  ...
  export default {
    ...
    self.order.orderItems = self.order.orderItems.map(item => {
      const selectedItem = self.itemsList.find(i => i.id === item.item);
      return {
        ...item,
        item: selectedItem ? selectedItem.name : item.item
      };
    });
    ...
    axios.post('api/orders', self.order)
        .then(function (response) {
            ...
            self.order.orderItems = [{ item: "", item_count: ""}]; // Reset items
      ...
      filteredItemsList(index) {
        const selectedItems = this.order.orderItems.map(item => item.item);
        return this.itemsList.filter(item => !selectedItems.includes(item.id) || item.id === this.order.orderItems[index].item);
      }
    }
  };
  </script>
