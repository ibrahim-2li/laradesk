...
export default {
    ...
    metaInfo() {
        return {
            title: this.$i18n.t('Update stock')
        }
    },
    data() {
        return {
    ...
    methods: {
        saveStock() {
            const self = this;
            self.loading = true;
            axios.put('api/dashboard/admin/stocks/' + self.$route.params.id, self.stock).then(function () {
                self.loading = false;
                self.$notify({
        ...
        getStock() {
            const self = this;
            self.loading = true;
            axios.get('api/dashboard/admin/stocks/' + self.$route.params.id).then(function (response) {
    ...
    /*    getStck() {
            const self = this;
            self.loading = true;
            axios.get('api/dashboard/admin/stocks/'+ self.$route.params.id)
