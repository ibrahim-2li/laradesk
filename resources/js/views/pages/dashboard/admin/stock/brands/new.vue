            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-5">
                <div class="md:flex md:items-center md:justify-between">
                    <div class="flex-1 min-w-0">
                        <h1 class="py-0.5 text-2xl font-semibold text-gray-900">{{ $t('Create Brand') }}</h1>
                    </div>
                </div>
            </div>
...
export default {
    ...
    metaInfo() {
        return {
            title: this.$i18n.t('Create Brand')
        }
    },
    data() {
        return {
    ...
    methods: {
        saveBrand() {
            const self = this;
            self.loading = true;
            axios.post('api/dashboard/admin/brands', self.brand).then(function (response) {
                self.loading = false;
                self.$notify({
                ...
                self.$router.push('/dashboard/admin/stock/brands/' + response.data.brand.id + '/edit');
            }).catch(function () {
                self.loading = false;
            });
        },
