    ...
    <main class="flex-1 relative overflow-y-auto py-6 focus:outline-none" tabindex="0">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-5">
            <div class="md:flex md:items-center md:justify-between">
                <div class="flex-1 min-w-0">
                    <h1 class="py-0.5 text-2xl font-semibold text-gray-900">{{ $t('Branches') }}</h1>
                </div>
                <div class="mt-4 flex md:mt-0 md:ml-4">
                    <router-link
                        class="btn btn-blue shadow-sm rounded-md"
                        to="/dashboard/admin/branches/new"
                    >
                        {{ $t('Create branche') }}
                    </router-link>
                </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="my-6 bg-white shadow overflow-hidden sm:rounded-md">
                ...
                <template v-if="brancheList.length > 0">
                    <ul>
                        <template v-for="(branche, index) in brancheList">
                            <li :class="{'border-t border-gray-200': index !== 0}">
                                <router-link
                                    :to="'/dashboard/admin/branches/' + branche.id + '/edit'"
                                        ...
                                        <div class="min-w-0 flex-1 sm:flex sm:items-center sm:justify-between">
                                            ...
                                            <div class="mt-4 flex-shrink-0 sm:mt-0">
                                                <div class="flex overflow-hidden">
                                                    ...
                                                    <template v-else-if="branche.all_agents && branche.agents.length === 0">
                                                        <div class="text-sm text-gray-700">
                                                            {{ $t('No agents assigned') }}
                                                        </div>
                                                    </template>
                                                    <template v-else>
                                                        <template v-for="(agent, index) in branche.agents">
                                                            <img
...
export default {
    ...
    methods: {
        getBranche() {
            const self = this;
            self.loading = true;
            axios.get('api/dashboard/admin/branches').then(function (response) {
                self.brancheList = response.data;
                self.loading = false;
            }).catch(function () {
                self.loading = false;
            });
