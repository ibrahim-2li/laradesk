            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-5">
                <div class="md:flex md:items-center md:justify-between">
                    <div class="flex-1 min-w-0">
                        <h1 class="py-0.5 text-2xl font-semibold text-gray-900">{{ $t('Create Branches') }}</h1>
                    </div>
                </div>
            </div>
                ...
                <div class="mt-6 shadow sm:rounded-lg">
                    ...
                    <div class="bg-white md:grid md:grid-cols-3 md:gap-6 px-4 py-5 sm:p-6">
                        <div class="md:col-span-1">
                            <h3 class="text-lg font-medium leading-6 text-gray-900">{{ $t('Branches details') }}</h3>
                            <p class="mt-1 text-sm leading-5 text-gray-500">
                                {{ $t('Branches details and settings') }}.
                            </p>
                        </div>
                            ...
                            <div class="grid grid-cols-3 gap-6">
                                ...
                                <div class="col-span-3">
                                    ...
                                    <div class="mt-2 relative text-xs text-gray-500">
                                        {{ $t('Allows access to the branches to all agents, or exclusively to a specific group of agents') }}.
                                    </div>
                                </div>
                                <div class="col-span-3">
                                    ...
                                    ></input-switch>
                                    <div class="mt-2 relative text-xs text-gray-500">
                                        {{ $t('If the branches is public, it allows users to select this branches when creating the ticket, otherwise only agents can reassign to this branches') }}.
                                    </div>
                                </div>
                            </div>
                        </div>
                        <template v-if="!branche.all_agents">
                            ...
                            <div class="md:col-span-1">
                                <h3 class="text-lg font-medium leading-6 text-gray-900">{{ $t('Branches agents') }}</h3>
                                <p class="mt-1 text-sm leading-5 text-gray-500">
                                    {{ $t('List of agents assigned to the branches') }}.
                                </p>
                            </div>
                            <div class="mt-5 md:mt-0 md:col-span-2">
                                <div class="grid grid-cols-3 gap-6">
...
export default {
    ...
    metaInfo() {
        return {
            title: this.$i18n.t('Create branches')
        }
    },
    data() {
        return {
    ...
    methods: {
        saveBranch() {
            const self = this;
            self.loading = true;
            axios.post('api/dashboard/admin/branches', self.branche).then(function (response) {
                self.loading = false;
                self.$notify({
                ...
                self.$router.push('/dashboard/admin/branches/' + response.data.branche.id + '/edit');
            }).catch(function () {
                self.loading = false;
            });
        },
